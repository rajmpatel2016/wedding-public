...
export const EditPhoneNumber: React.FC<EditPhoneNumberProps> = ({
  close,
  open,
}) => {
  ...
  useEffect(() => {
    if (container.user.phone) {
      setPhone(container.user.phone);
    }
  }, [container.user]);

  const savePhone = () => {
    container.editUserPhone({ phone, callback: close });
  };
